...
Java_com_example_sd_1flutter_1android_SdFlutterAndroidPlugin_initModel(
    ...
    // free_params_immediately was removed in newer stable-diffusion.cpp API.
...
SD_FFI_API void* sd_ffi_init_ex(const char* model_path,
    ...
    // offload_params_to_cpu, keep_vae_on_cpu, free_params_immediately removed in newer API.
    (void)offload_params_to_cpu;
    (void)keep_vae_on_cpu;
    char max_vram_str[32] = {0};
    if (max_vram > 0.0f) {
        snprintf(max_vram_str, sizeof(max_vram_str), "%.2f", max_vram);
        params.max_vram = max_vram_str;
    }
...
SD_FFI_API uint8_t* sd_ffi_generate(void* ctx,
    ...
    *out_size = 0;
    ...
    if (!gen_ok || !images_out || num_images_out == 0) {
